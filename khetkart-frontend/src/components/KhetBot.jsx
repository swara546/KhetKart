...
const FARMER_SUGGESTIONS = [
  ...
  "What is the MSP for rice 2025?",
  "Tips for drip irrigation?",
  "How to store onions after harvest?",
];

const VENDOR_SUGGESTIONS = [
  "Which crops are available in winter?",
  "How to check quality of wheat before buying?",
  "Best pulses to buy in bulk?",
  "How to store tomatoes longer?",
...
function getWelcome(role) {
  if (role === "farmer")
    return "Jai Kisan! 🌾 I'm KhetBot, your farming assistant.\n\nAsk me about crops, pest control, irrigation, MSP prices, or government schemes!";
  if (role === "vendor")
    return "Namaste! 🏪 I'm KhetBot, your procurement assistant.\n\nAsk me about crop availability, quality checks, storage tips, or seasonal pricing!";
  return "Jai Kisan! 🌾 I'm KhetBot, your farming assistant.\n\nAsk me anything about Indian agriculture or KhetKart!";
}
  ...
  return (
    <div className={`flex ${isBot ? "justify-start" : "justify-end"} mb-3`}>
      ...
      <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed shadow-sm ${
        ...
        {msg.content.split("\n").map((line, i) => (
          <p key={i} className={line.startsWith("-") ? "ml-2" : ""}>{line || <br />}</p>
        ))}
      </div>
    </div>
  );
...
function KhetBot() {
  ...
  const [messages, setMessages] = useState([
    { role: "assistant", content: getWelcome(role) },
  ]);
  ...
  // Reset welcome message when user logs in/out
  useEffect(() => {
    setMessages([{ role: "assistant", content: getWelcome(role) }]);
  }, [role]);
  ...
  const suggestions =
    role === "farmer" ? FARMER_SUGGESTIONS :
    role === "vendor" ? VENDOR_SUGGESTIONS :
    GUEST_SUGGESTIONS;
  ...
  const sendMessage = async (text) => {
    ...
    try {
      const res = await axios.post("/api/ai/chat", {
        ...
        role,   // ← send user role to backend
      });
  ...
  return (
    <>
      {open && (
        <div className="fixed bottom-24 right-4 sm:right-6 z-50 w-[340px] sm:w-[380px] flex flex-col bg-gray-50 rounded-3xl shadow-2xl border border-gray-200 overflow-hidden"
          style={{ height: "520px" }}>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-green-200 text-xs">
                  {role === "farmer" ? "🧑‍🌾 Farmer Assistant" : role === "vendor" ? "🏪 Vendor Assistant" : "AI Farming Assistant"}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={clearChat} className="text-green-300 hover:text-white text-xs transition px-2 py-1 rounded-lg hover:bg-white/10">
                Clear
              </button>
              <button onClick={() => setOpen(false)} className="text-green-300 hover:text-white transition p-1 rounded-lg hover:bg-white/10">
                <MdClose size={20} />
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
            ...
            {loading && (
              <div className="flex justify-start mb-3">
                <div className="w-7 h-7 rounded-full bg-green-700 flex items-center justify-center text-white text-xs shrink-0 mr-2 mt-0.5">🌾</div>
                <TypingIndicator />
              </div>
            )}
          ...
          <div className="px-3 pb-3 pt-2 border-t border-gray-100 bg-white shrink-0">
            <div className="flex items-end gap-2 bg-gray-50 border border-gray-200 rounded-2xl px-3 py-2 focus-within:border-green-400 focus-within:ring-2 focus-within:ring-green-100 transition">
              <textarea ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)}
                ...
                placeholder={role === "vendor" ? "Ask about crops, quality, storage…" : "Ask about crops, pests, prices…"}
                rows={1}
            ...
            <p className="text-center text-xs text-gray-300 mt-1.5">Powered by Groq AI</p>
          </div>
        </div>
      )}
      ...
      <button onClick={() => setOpen((o) => !o)}
        className={`fixed bottom-6 right-4 sm:right-6 z-50 w-14 h-14 rounded-full shadow-lg flex items-center justify-center text-2xl transition-all duration-300 ${
          open ? "bg-gray-700 hover:bg-gray-600" : "bg-green-700 hover:bg-green-600 hover:scale-110"
        }`}
        title="KhetBot — AI Farming Assistant">
        {open ? <MdClose size={24} className="text-white" /> : <MdSmartToy size={26} className="text-white" />}
      </button>
