...
const ROLES = [
  {
    value: "farmer",
    label: "🧑‍🌾 Farmer",
    desc: "I grow crops and want to sell directly to vendors",
...
function Register() {
  ...
  const [formData, setFormData] = useState({
    name: "", mobile: "", village: "", role: "farmer", password: "", confirmPassword: "",
  });
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // Farmers go to dashboard, vendors go to browse crops
      navigate(formData.role === "farmer" ? "/seller-dashboard" : "/products");
    } catch (err) {
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-green-100">
          ...
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-extrabold text-green-800 mt-3">Join KhetKart</h2>
            <p className="text-sm text-gray-400 mt-1">India's direct farm-to-vendor marketplace</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
                ...
                {ROLES.map(({ value, label, desc, color }) => (
                  <button type="button" key={value}
                    ...
                    className={`border-2 rounded-xl p-3 text-left transition ${
                      formData.role === value ? color : "border-gray-200 hover:border-gray-300"
                    }`}>
            ...
            <div>
              ...
              <input name="name"
                placeholder={formData.role === "farmer" ? "Ramesh Patil" : "Suresh Traders"}
            ...
            <div>
              ...
              <div className="flex">
                ...
                <input name="mobile" type="tel" maxLength={10} placeholder="9876543210"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                {formData.role === "farmer" ? "Village / District" : "City / Business Location"}
              </label>
              <input name="village"
                placeholder={formData.role === "farmer" ? "Nashik, Maharashtra" : "Pune, Maharashtra"}
                onChange={handleChange} className={inputClass} />
            </div>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? "Creating account…" : `Join as ${formData.role === "farmer" ? "Farmer 🌾" : "Vendor 🏪"} →`}
            </button>
          </form>
