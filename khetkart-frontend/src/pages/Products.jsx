...
// ── Fix 3: Start qty at minOrder, increment by minOrder ──────────
function saveToCart(product) {
  const cart = JSON.parse(localStorage.getItem("khetkart_cart") || "[]");
  const minQty = product.minOrder || 1;
  const existing = cart.find((i) => i._id === product._id);
  if (existing) {
    // increment by minOrder step, don't exceed stock
    const newQty = existing.qty + minQty;
    existing.qty = product.stock ? Math.min(newQty, product.stock) : newQty;
  } else {
    // start at minOrder
    cart.push({ ...product, qty: minQty });
  }
          ...
          <div>
            ...
            {product.minOrder && (
              <p className="text-xs text-orange-500">
                Min: {product.minOrder} {product.unit || "kg"}
              </p>
            )}
          </div>
          <button
    ...
    >
      ...
      >
        ...
        <div className="p-6">
          ...
          <h2 className="text-xl font-extrabold text-gray-800 mt-1 mb-2">
            {product.name}
          </h2>
            ...
            {product.minOrder && (
              <div className="bg-orange-50 rounded-xl p-3">
                ...
                <p className="font-semibold text-orange-600">
                  {product.minOrder} {product.unit || "kg"}
                </p>
              </div>
            )}
            {product.rating > 0 && (
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-6 flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search wheat, tomato, mango…"
              value={search}
